...
function getInstagramInfoFallback(url) {
  return new Promise((resolve, reject) => {
    const postData = JSON.stringify({ url: url, iphone: false });
    const options = {
      method: 'POST',
      headers: {
    ...
    console.log(`[Instagram fallback] Initiating job on Publer for: ${url}`);
    requestWithRedirect('https://app.publer.io/hooks/media', options, postData)
      .then(res => {
        if (res.statusCode !== 200) {
          return reject(new Error(`Publer job creation failed: HTTP ${res.statusCode}`));
        }
        const json = JSON.parse(res.data);
        const jobId = json.job_id || json.id;
        if (!jobId) {
          return reject(new Error('No job ID returned by Publer'));
        }

        pollPublerJob(jobId, url, resolve, reject);
      })
      .catch(err => {
        reject(err);
      });
...
function pollPublerJob(jobId, originalUrl, resolve, reject, attempts = 0) {
  if (attempts > 15) {
    return reject(new Error('Publer job timed out'));
  }
  ...
  requestWithRedirect(`https://app.publer.com/api/v1/job_status/${jobId}`, options)
    .then(res => {
      const json = JSON.parse(res.data);
      if (json.status === 'complete') {
        const payload = json.payload;
        if (!Array.isArray(payload) || payload.length === 0) {
          return reject(new Error('Empty payload returned by Publer'));
        }

        const item = payload[0];
        const mediaUrl = item.url;
        if (!mediaUrl) {
          return reject(new Error('No media URL in Publer payload'));
        }

        const base64Url = Buffer.from(mediaUrl).toString('base64url');
        const isAudio = item.type === 'audio';

        const formats = [{
          formatId: `fb_${base64Url}`,
          ext: isAudio ? 'mp3' : 'mp4',
          resolution: isAudio ? 'audio only' : '720p',
          width: 0,
          height: 0,
          filesize: null,
          hasAudio: true,
          hasVideo: !isAudio,
          qualityLabel: isAudio ? 'MP3 Audio' : 'HD Video',
          vcodec: isAudio ? 'none' : 'h264',
          acodec: isAudio ? 'mp3' : 'aac',
        }];

        resolve({
          title: item.desc || 'Instagram Video',
        ...
        reject(new Error('Publer processing job failed'));
      } else {
        setTimeout(() => {
          pollPublerJob(jobId, originalUrl, resolve, reject, attempts + 1);
        }, 1000);
      }
    })
    .catch(err => {
...
function handleInstagramDownloadFallback(url, formatId, headers, res) {
  getInstagramInfoFallback(url)
    .then(info => {
      const fallbackFormat = info.formats[0];
      if (!fallbackFormat || !fallbackFormat.formatId) {
        throw new Error('No formats resolved in fallback');
      }
      const targetUrl = Buffer.from(fallbackFormat.formatId.substring(3), 'base64url').toString('utf8');
      
      console.log(`[Instagram fallback stream] Streaming from: ${targetUrl}`);
      
      res.setHeader('Content-Type', headers.contentType);
      res.setHeader('Content-Disposition', headers.contentDisposition);

      https.get(targetUrl, (streamRes) => {
        if (streamRes.headers['content-length']) {
          res.setHeader('Content-Length', streamRes.headers['content-length']);
        }
        streamRes.pipe(res);
      }).on('error', (err) => {
        console.error(`[Instagram fallback stream] Error: ${err.message}`);
        if (!res.headersSent) {
          res.status(500).json({ success: false, error: 'Failed to stream video' });
        }
      });
    ...
    .catch(err => {
      console.error(`[Instagram download fallback] Failed: ${err.message}`);
      if (!res.headersSent) {
        res.status(500).json({ success: false, error: 'Download failed' });
      }
    });

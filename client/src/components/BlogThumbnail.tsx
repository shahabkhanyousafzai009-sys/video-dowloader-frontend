...
interface BlogThumbnailProps {
  category: 'TikTok' | 'Instagram' | 'Facebook' | 'MP3 Conversion' | 'Social Media Guides' | 'Legal & Security' | string;
  title: string;
  imageUrl?: string;
  size?: 'sm' | 'md' | 'lg';
}
...
export const BlogThumbnail: React.FC<BlogThumbnailProps> = ({ category, title, imageUrl, size = 'md' }) => {
  ...
  const getCategoryConfig = () => {
    switch (category) {
      ...
      case 'Social Media Guides':
        return {
          gradient: 'from-sky-700 via-slate-900 to-blue-950',
          badgeText: 'MEDIA GUIDES',
          badgeBg: 'bg-black/60 text-sky-300 border-sky-500/40',
          icon: (
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
